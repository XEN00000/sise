...
def run_dfs_for_permutation(permutation):
    ...
    for filename in puzzles:
        ...
        file_count = len(
            [f for f in os.listdir(folder_with_puzzles) if os.path.isfile(os.path.join(folder_with_puzzles, f))])
        counter += 1
...
if __name__ == "__main__":
    ...
    # Uruchamiamy BFS i A* normalnie
    run_other_algorithms()

    # Uruchomiamy DFS na 8 wątkach (każdy z osobną permutacją)
